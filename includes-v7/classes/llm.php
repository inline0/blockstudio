...
class LLM
{
    ...
    public static function getTxtData(): string
    {
        ...
        $docs = file_get_contents(
            BLOCKSTUDIO_DIR . '/includes-v7/llm/blockstudio.md'
        );
        $blockSchema = json_encode(
            json_decode(
                file_get_contents(
                    BLOCKSTUDIO_DIR . '/includes-v7/schemas/block.json'
                )
            )
        );
        $extensionsSchema = json_encode(
            json_decode(
                file_get_contents(
                    BLOCKSTUDIO_DIR . '/includes-v7/schemas/extensions.json'
                )
            )
        );
        ...
        $exampleBlocksBasePath =
            BLOCKSTUDIO_DIR . '/includes-v7/library/blockstudio-element/';
