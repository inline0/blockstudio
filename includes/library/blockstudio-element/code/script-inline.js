/* PrismJS 1.29.0
https://prismjs.com/download.html#themes=prism&languages=markup+css+clike+javascript+json+markup-templating+php+jsx+scss+twig&plugins=line-highlight+line-numbers+normalize-whitespace */
var _self =
...
function init(code) {
  ...
  const copy = code.querySelector("[role='button']");
  let originalContent = codeInner.textContent;
  let highlightedContent;

  let content = Prism.plugins.NormalizeWhitespace.normalize(originalContent, {
  ...
  codeInner.innerHTML = highlightedContent;

  const observer = new MutationObserver((mutationsList, observer) => {
    observer.disconnect();
    resetToHighlighted();
    observer.observe(codeInner, { childList: true, subtree: true });
  });

  const resetToHighlighted = () => {
    codeInner.innerHTML = highlightedContent;
  };
...
function initAll() {
  document.querySelectorAll(".blockstudio-element-code").forEach(init);
}

["DOMContentLoaded", "blockstudio/blockstudio-element/code/loaded"].forEach(
