...
namespace BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\Vcs;

use BlockstudioVendor\Parsedown;
if (!class_exists(GitHubApi::class, \false)) {
    class GitHubApi extends Api
    {
        ...
        public function getLatestRelease()
        {
            ...
            if ($this->shouldSkipPreReleases() && ($this->releaseFilterMaxReleases === 1 || !$this->hasCustomReleaseFilter())) {
                ...
                $foundReleases = $this->api('/repos/:user/:repo/releases', array('per_page' => $this->releaseFilterMaxReleases));
            ...
            foreach ($foundReleases as $release) {
                ...
                if ($this->shouldSkipPreReleases() && isset($release->prerelease) && !empty($release->prerelease)) {
                    continue;
                }
                $versionNumber = ltrim($release->tag_name, 'v');
                //Remove the "v" prefix from "v1.2.3".
                //Custom release filtering.
                if (!$this->matchesCustomReleaseFilter($versionNumber, $release)) {
                    continue;
                }
                $reference = new Reference(array('name' => $release->tag_name, 'version' => $versionNumber, 'downloadUrl' => $release->zipball_url, 'updated' => $release->created_at, 'apiResponse' => $release));
        ...
        public function getLatestTag()
        {
            ...
            $tag = $versionTags[0];
            return new Reference(array('name' => $tag->name, 'version' => ltrim($tag->name, 'v'), 'downloadUrl' => $tag->zipball_url, 'apiResponse' => $tag));
        }
        /**
        ...
        public function getBranch($branchName)
        {
            ...
            $reference = new Reference(array('name' => $branch->name, 'downloadUrl' => $this->buildArchiveDownloadUrl($branch->name), 'apiResponse' => $branch));
        ...
        {
            return sprintf('//api.github.com/repos/%1$s/%2$s/releases/assets/', $this->userName, $this->repositoryName);
        }
