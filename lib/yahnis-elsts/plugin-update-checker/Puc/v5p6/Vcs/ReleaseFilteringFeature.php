...
if (!trait_exists(ReleaseFilteringFeature::class, \false)) {
    trait ReleaseFilteringFeature
    {
         ...
         * @return $this
         */
        public function setReleaseFilter($callback, $releaseTypes = Api::RELEASE_FILTER_SKIP_PRERELEASE, $maxReleases = 20)
        {
            if ($maxReleases > 100) {
                throw new \InvalidArgumentException(sprintf('The max number of releases is too high (%d). It must be 100 or less.', $maxReleases));
            } else if ($maxReleases < 1) {
                throw new \InvalidArgumentException(sprintf('The max number of releases is too low (%d). It must be at least 1.', $maxReleases));
            }
         ...
         */
        public function setReleaseVersionFilter($regex, $releaseTypes = Api::RELEASE_FILTER_SKIP_PRERELEASE, $maxReleasesToExamine = 20)
        {
            return $this->setReleaseFilter(function ($versionNumber) use ($regex) {
                return preg_match($regex, $versionNumber) === 1;
            }, $releaseTypes, $maxReleasesToExamine);
        }
        /**
