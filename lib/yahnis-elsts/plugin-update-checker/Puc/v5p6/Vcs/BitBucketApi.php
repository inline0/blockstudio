...
use BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\OAuthSignature;
use BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\Utils;
if (!class_exists(BitBucketApi::class, \false)) {
    class BitBucketApi extends Api
    {
        ...
        {
            $strategies = array(self::STRATEGY_STABLE_TAG => function () use ($configBranch) {
        ...
        public function getBranch($branchName)
        {
            ...
            return new Reference(array('name' => $ref, 'updated' => $branch->target->date, 'downloadUrl' => $this->getDownloadUrl($branch->name)));
        }
        /**
         * Get a specific tag.
         *
        ...
        public function getTag($tagName)
        {
            ...
            return new Reference(array('name' => $tag->name, 'version' => ltrim($tag->name, 'v'), 'updated' => $tag->target->date, 'downloadUrl' => $this->getDownloadUrl($tag->name)));
        }
        /**
        ...
        public function setAuthentication($credentials)
        {
            ...
            if (!empty($credentials) && !empty($credentials['consumer_key'])) {
                $this->oauth = new OAuthSignature($credentials['consumer_key'], $credentials['consumer_secret']);
            } else {
                $this->oauth = null;
            }
        }
