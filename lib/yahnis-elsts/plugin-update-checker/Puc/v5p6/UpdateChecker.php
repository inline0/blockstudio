...
if (!class_exists(UpdateChecker::class, \false)) {
    abstract class UpdateChecker
    {
        ...
        public function excludeEntityFromWordPressAPI($args, $url)
        {
            ...
            $expectedPathPrefix = '/' . $typePluralised . '/update-check/1.';
            //e.g. "/plugins/update-check/1.1/"
            ...
            if (!apply_filters($this->getUniqueName('remove_from_default_update_checks'), \true, $this, $args, $url)) {
                return $args;
            }
            if (empty($args['body'][$typePluralised])) {
                return $args;
            }
            $reportingItems = json_decode($args['body'][$typePluralised], \true);
        ...
        protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array())
        {
            ...
            $options = array('timeout' => wp_doing_cron() ? 10 : 3, 'headers' => array('Accept' => 'application/json'));
                ...
                if (!$wp_filesystem->is_dir($newDirectory)) {
                    ...
                    if (is_array($sourceFiles)) {
                        ...
                        foreach ($sourceFiles as $filename) {
                            ...
                            if (!$wp_filesystem->move($previousSource, $newSource, \true)) {
                                $allMoved = \false;
                                break;
                            }
                        }
                        if ($allMoved) {
                            ...
                            $wp_filesystem->rmdir($newDirectory, \true);
                            //And return a relevant error.
                            return new WP_Error('puc-incorrect-directory-structure', sprintf('The directory structure of the update was incorrect. All files should be inside ' . 'a directory named <span class="code">%s</span>, not at the root of the ZIP archive. Plugin Update Checker tried to fix the directory structure, but failed.', htmlentities($this->slug)));
                        }
                    }
                }
            }
        ...
        {
            if (class_exists('BlockstudioVendor\Debug_Bar', \false) && class_exists('BlockstudioVendor\Debug_Bar_Panel', \false) && file_exists(dirname(__FILE__) . '/DebugBar')) {
