...
namespace BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\DebugBar;

use BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\UpdateChecker;
if (!class_exists(Panel::class, \false) && class_exists('BlockstudioVendor\Debug_Bar_Panel', \false)) {
    class Panel extends \BlockstudioVendor\Debug_Bar_Panel
    {
        /** @var UpdateChecker */
        ...
        {
            printf('<div class="puc-debug-bar-panel-v5" id="%1$s" data-slug="%2$s" data-uid="%3$s" data-nonce="%4$s">', esc_attr($this->updateChecker->getUniqueName('debug-bar-panel')), esc_attr($this->updateChecker->slug), esc_attr($this->updateChecker->getUniqueName('uid')), esc_attr(wp_create_nonce('puc-ajax')));
        ...
        private function displayStatus()
        {
            ...
            if (function_exists('get_submit_button')) {
                ...
                $checkNowButton = sprintf('<input type="button" id="%1$s" name="puc-check-now-button" value="%2$s" class="button button-secondary" />', esc_attr($checkButtonId), esc_attr('Check Now'));
            }
            ...
            if ($update !== null) {
                ...
                foreach ($fields as $field) {
                    if (property_exists($update, $field)) {
                        $this->row(ucwords(str_replace('_', ' ', $field)), isset($update->{$field}) ? htmlentities($update->{$field}) : null);
                    }
                }
                echo '</table>';
            } else {
