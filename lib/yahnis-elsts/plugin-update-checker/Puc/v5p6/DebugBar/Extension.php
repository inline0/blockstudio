...
use BlockstudioVendor\YahnisElsts\PluginUpdateChecker\v5p6\PucFactory;
...
if (!class_exists(Extension::class, \false)) {
    class Extension
    {
        ...
        {
            wp_enqueue_style('puc-debug-bar-style-v5', $this->getLibraryUrl("/css/puc-debug-bar.css"), array('debug-bar'), '20221008');
            wp_enqueue_script('puc-debug-bar-js-v5', $this->getLibraryUrl("/js/debug-bar.js"), array('jquery'), '20221008');
        }
        /**
        ...
        public function ajaxCheckNow()
        {
            ...
            if ($update !== null) {
                ...
                echo '<pre>', esc_html(print_r($update, \true)), '</pre>';
            } else {
                ...
                foreach (array_values($errors) as $num => $item) {
                    ...
                    if (isset($item['httpResponse'])) {
                        if (is_wp_error($item['httpResponse'])) {
                            ...
                            //Status code.
                            printf('<dt>HTTP status:</dt><dd><code>%d %s</code></dd>', esc_html(wp_remote_retrieve_response_code($item['httpResponse'])), esc_html(wp_remote_retrieve_response_message($item['httpResponse'])));
                            //Headers.
