...
use BlockstudioVendor\ScssPhp\ScssPhp\Value\ColorFormatEnum;
...
class ColorFunctions
{
    ...
    {
        return self::updateComponents($arguments, adjust: \true);
    }
    /**
     * @param list<Value> $arguments
     */
    ...
    {
        return self::updateComponents($arguments, scale: \true);
    }
    /**
     * @param list<Value> $arguments
     */
    ...
    {
        return self::updateComponents($arguments, change: \true);
    }
    /**
     * @param list<Value> $arguments
     */
...
, Deprecation::functionUnits);
            ...
            return $scale || $assertPercent ? $number->valueInRange($change ? 0 : -$max, $max, $name) : $number->valueInRangeWithUnit($change ? 0 : -$max, $max, $name, $checkPercent ? '%' : '');
        };
        $alpha = $getParam('alpha', 1, checkUnitless: \true);
        ...
        $saturation = $getParam('saturation', 100, checkPercent: \true);
        $lightness = $getParam('lightness', 100, checkPercent: \true);
        $whiteness = $getParam('whiteness', 100, assertPercent: \true);
        $blackness = $getParam('blackness', 100, assertPercent: \true);
        if (\count($keywords) > 0) {
            throw new SassScriptException(sprintf('No %s named %s.', StringUtil::pluralize('argument', \count($keywords)), StringUtil::toSentence(array_map(fn($name) => "\${$name}", array_keys($keywords)), 'or')));
        }
        ...
        if ($hasRgb) {
            return $color->changeRgb($updateRgb($color->getRed(), $red), $updateRgb($color->getGreen(), $green), $updateRgb($color->getBlue(), $blue), $updateValue($color->getAlpha(), $alpha, 1));
        }
        if ($hasWB) {
            return $color->changeHwb($change ? $hue : $color->getHue() + ($hue ?? 0), $updateValue($color->getWhiteness(), $whiteness, 100), $updateValue($color->getBlackness(), $blackness, 100), $updateValue($color->getAlpha(), $alpha, 1));
        }
        if ($hue !== null || $hasSL) {
            return $color->changeHsl($change ? $hue : $color->getHue() + ($hue ?? 0), $updateValue($color->getSaturation(), $saturation, 100), $updateValue($color->getLightness(), $lightness, 100), $updateValue($color->getAlpha(), $alpha, 1));
        }
        if ($alpha !== null) {
    ...
    private static function hslImpl(string $name, array $arguments): Value
    {
        ...
        return SassColor::hslInternal($hue, NumberUtil::clamp($saturation->getValue(), 0, 100), NumberUtil::clamp($lightness->getValue(), 0, 100), $alpha !== null ? self::percentageOrUnitless($alpha->assertNumber('alpha'), 1, 'alpha') : 1, ColorFormatEnum::hslFunction);
    }
    /**
...
, Deprecation::functionUnits);
    ...
    private static function parseChannels(string $name, array $argumentNames, Value $channels): SassString|array
    {
        ...
        if ($channels->getSeparator() === ListSeparator::SLASH) {
            ...
            if (\count($list) !== 2) {
                throw new SassScriptException(sprintf('Only 2 slash-separated elements allowed, but %s %s passed.', \count($list), StringUtil::pluralize('was', \count($list), 'were')));
            }
            $channels = $list[0];
    ...
    private static function mixColors(SassColor $color1, SassColor $color2, SassNumber $weight): SassColor
    {
        ...
        $combinedWeight1 = $normalizedWeight * $alphaDistance == -1 ? $normalizedWeight : ($normalizedWeight + $alphaDistance) / (1 + $normalizedWeight * $alphaDistance);
        $weight1 = ($combinedWeight1 + 1) / 2;
        $weight2 = 1 - $weight1;
        return SassColor::rgb(NumberUtil::fuzzyRound($color1->getRed() * $weight1 + $color2->getRed() * $weight2), NumberUtil::fuzzyRound($color1->getGreen() * $weight1 + $color2->getGreen() * $weight2), NumberUtil::fuzzyRound($color1->getBlue() * $weight1 + $color2->getBlue() * $weight2), $color1->getAlpha() * $weightScale + $color2->getAlpha() * (1 - $weightScale));
    }
    /**
     * @param list<Value> $arguments
     */
