...
use BlockstudioVendor\ScssPhp\ScssPhp\Evaluation\EvaluationContext;
...
use BlockstudioVendor\ScssPhp\ScssPhp\Util\ArrayUtil;
...
final class SelectorFunctions
{
    ...
    public static function append(array $arguments): Value
    {
        ...
        return ArrayUtil::reduce(array_map(fn(Value $selector) => $selector->assertSelector(), $selectors), function (SelectorList $parent, SelectorList $child) use ($span) {
            return (new SelectorList(array_map(function (ComplexSelector $complex) use ($span, $parent) {
                ...
                return new ComplexSelector([], [new ComplexSelectorComponent($newCompound, $component->getCombinators(), $span), ...$rest], $span);
    ...
    public static function simpleSelectors(array $arguments): Value
    {
        ...
        return new SassList(array_map(fn(SimpleSelector $simple) => new SassString((string) $simple, \false), $selector->getComponents()), ListSeparator::COMMA);
    }
    /**
     * @param list<Value> $arguments
     */
    ...
    private static function prependParent(CompoundSelector $compound): ?CompoundSelector
    {
        ...
        if ($firstComponent instanceof TypeSelector) {
            return new CompoundSelector([new ParentSelector($span, $firstComponent->getName()->getName()), ...array_slice($compound->getComponents(), 1)], $span);
        }
        return new CompoundSelector([new ParentSelector($span), ...$compound->getComponents()], $span);
    }
}
