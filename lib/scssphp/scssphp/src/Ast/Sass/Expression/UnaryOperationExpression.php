...
final class UnaryOperationExpression implements Expression
{
    ...
    public function __toString(): string
    {
        ...
        $needsParens = $this->operand instanceof BinaryOperationExpression || $this->operand instanceof UnaryOperationExpression || $this->operand instanceof ListExpression && !$this->operand->hasBrackets() && \count($this->operand->getContents()) > 1;
        if ($needsParens) {
            $buffer .= '(';
        }
        $buffer .= $this->operand;
