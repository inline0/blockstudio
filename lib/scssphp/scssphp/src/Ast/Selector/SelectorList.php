 ...
 */
namespace BlockstudioVendor\ScssPhp\ScssPhp\Ast\Selector;
...
use BlockstudioVendor\ScssPhp\ScssPhp\Extend\ExtendUtil;
...
final class SelectorList extends Selector
{
     ...
     */
    public static function parse(string $contents, ?LoggerInterface $logger = null, ?InterpolationMap $interpolationMap = null, ?UriInterface $url = null, bool $allowParent = \true, bool $plainCss = \false): SelectorList
    {
     ...
     */
    public function nestWithin(?SelectorList $parent, bool $implicitParent = \true, bool $preserveParentSelectors = \false): SelectorList
    {
        if ($parent === null) {
            ...
            foreach ($complex->getComponents() as $component) {
                ...
                if ($resolved === null) {
                    ...
                    if (\count($complex->getLeadingCombinators()) === 0) {
                        ...
                        $newComplexes = array_map(fn(ComplexSelector $resolvedComplex) => new ComplexSelector(array_merge($complex->getLeadingCombinators(), $resolvedComplex->getLeadingCombinators()), $resolvedComplex->getComponents(), $complex->getSpan(), $resolvedComplex->getLineBreak()), $resolved);
                    }
                } else {
    ...
    private static function nestWithinCompound(ComplexSelectorComponent $component, SelectorList $parent): ?array
    {
        ...
        if (!$parentSelector instanceof ParentSelector) {
            return [new ComplexSelector([], [new ComplexSelectorComponent(new CompoundSelector($resolvedSimples, $component->getSelector()->getSpan()), $component->getCombinators(), $component->getSpan())], $component->getSpan())];
        }
        ...
        return array_map(function (ComplexSelector $complex) use ($parentSelector, $resolvedSimples, $component) {
            ...
            if ($suffix !== null) {
                $last = new CompoundSelector(array_merge(ListUtil::exceptLast($lastSimples), [ListUtil::last($lastSimples)->addSuffix($suffix)], array_slice($resolvedSimples, 1)), $component->getSelector()->getSpan());
            } else {
