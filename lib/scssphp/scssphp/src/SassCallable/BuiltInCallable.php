 ...
 */
namespace BlockstudioVendor\ScssPhp\ScssPhp\SassCallable;
...
class BuiltInCallable implements SassCallable
{
    ...
    {
        return self::parsed($name, ArgumentDeclaration::parse("@function {$name}({$arguments}) {", url: $url), $callback);
    }
    /**
    ...
    {
        return self::parsed($name, ArgumentDeclaration::parse("@mixin {$name}({$arguments}) {", url: $url), function ($arguments) use ($callback) {
            $callback($arguments);
    ...
    public static function overloadedFunction(string $name, array $overloads, ?UriInterface $url = null): BuiltInCallable
    {
        ...
        foreach ($overloads as $args => $callback) {
            $processedOverloads[] = [ArgumentDeclaration::parse("@function {$name}({$args}) {", url: $url), $callback];
        }
