...
abstract class ReplaceExpressionVisitor implements ExpressionVisitor
{
    ...
    {
        return new FunctionExpression($node->getOriginalName(), $this->visitArgumentInvocation($node->getArguments()), $node->getSpan(), $node->getNamespace());
    }
    public function visitInterpolatedFunctionExpression(InterpolatedFunctionExpression $node): Expression
    {
        return new InterpolatedFunctionExpression($this->visitInterpolation($node->getName()), $this->visitArgumentInvocation($node->getArguments()), $node->getSpan());
    }
    ...
    {
        return new ListExpression(array_map(fn(Expression $item) => $item->accept($this), $node->getContents()), $node->getSeparator(), $node->getSpan(), $node->hasBrackets());
    }
    public function visitMapExpression(MapExpression $node): Expression
    {
        return new MapExpression(array_map(fn(array $pair) => [$pair[0]->accept($this), $pair[1]->accept($this)], $node->getPairs()), $node->getSpan());
    }
    ...
    {
        return new ArgumentInvocation(array_map(fn(Expression $expression) => $expression->accept($this), $invocation->getPositional()), array_map(fn(Expression $expression) => $expression->accept($this), $invocation->getNamed()), $invocation->getSpan(), $invocation->getRest()?->accept($this), $invocation->getKeywordRest()?->accept($this));
    }
    /**
    ...
    protected function visitSupportsCondition(SupportsCondition $condition): SupportsCondition
    {
        if ($condition instanceof SupportsOperation) {
            return new SupportsOperation($this->visitSupportsCondition($condition->getLeft()), $this->visitSupportsCondition($condition->getRight()), $condition->getOperator(), $condition->getSpan());
        }
