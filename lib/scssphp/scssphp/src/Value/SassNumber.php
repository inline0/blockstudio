...
abstract class SassNumber extends Value
{
    ...
    {
        return $this->convertOrCoerceValue($newNumeratorUnits, $newDenominatorUnits, \true, $name);
    }
    /**
    ...
    {
        return $this->convertOrCoerceValue($other->getNumeratorUnits(), $other->getDenominatorUnits(), \true, $name, $other, $otherName);
    }
    /**
    ...
    public function equals(object $other): bool
    {
        ...
        if (self::canonicalizeUnitList($this->getNumeratorUnits()) !== self::canonicalizeUnitList($other->getNumeratorUnits()) || self::canonicalizeUnitList($this->getDenominatorUnits()) !== self::canonicalizeUnitList($other->getDenominatorUnits())) {
            return \false;
        }
        return NumberUtil::fuzzyEquals($this->value * self::getCanonicalMultiplier($this->getNumeratorUnits()) / self::getCanonicalMultiplier($this->getDenominatorUnits()), $other->value * self::getCanonicalMultiplier($other->getNumeratorUnits()) / self::getCanonicalMultiplier($other->getDenominatorUnits()));
    }
    /**
    ...
    private function compatibilityException(bool $otherHasUnits, array $newNumeratorUnits, array $newDenominatorUnits, ?string $name, ?SassNumber $other = null, ?string $otherName = null): SassScriptException
    {
        ...
        if (\count($newNumeratorUnits) === 1 && \count($newDenominatorUnits) === 0 && isset(self::TYPES_BY_UNIT[$newNumeratorUnits[0]])) {
            ...
            $article = \in_array($type[0], ['a', 'e', 'i', 'o', 'u'], \true) ? 'an' : 'a';
    ...
    {
        $result = "\${$name}" . implode(array_map(fn($unit) => " * 1{$unit}", $this->getDenominatorUnits())) . implode(array_map(fn($unit) => " / 1{$unit}", $this->getNumeratorUnits())) . ($unit === null ? '' : " * 1{$unit}");
