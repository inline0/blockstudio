...
use BlockstudioVendor\ScssPhp\ScssPhp\Ast\Selector\Combinator;
...
use BlockstudioVendor\ScssPhp\ScssPhp\Parser\LineScanner;
use BlockstudioVendor\ScssPhp\ScssPhp\Parser\Parser;
use BlockstudioVendor\ScssPhp\ScssPhp\Parser\StringScanner;
...
use BlockstudioVendor\ScssPhp\ScssPhp\Visitor\SelectorVisitor;
...
final class SerializeVisitor implements CssVisitor, ValueVisitor, SelectorVisitor
{
    ...
    public function __construct(bool $inspect = \false, bool $quote = \true, OutputStyle $style = OutputStyle::EXPANDED, bool $sourceMap = \false, ?LoggerInterface $logger = null)
    {
    ...
    public function visitCssDeclaration(CssDeclaration $node): void
    {
        if ($node->getInterleavedRules() !== []) {
            ...
            foreach ($node->getInterleavedRules() as $rule) {
                ...
                if (!IterableUtil::any($declSpecificities, fn($s) => \in_array($s, $ruleSpecificities, \true))) {
                    continue;
                }
                LoggerUtil::warnForDeprecation($this->logger, Deprecation::mixedDecls, <<<'MESSAGE'
...
MESSAGE
, new MultiSpan($node->getSpan(), 'declaration', ['nested rule' => $rule->getSpan()]), $node->getTrace());
            }
        }
        $this->writeIndentation();
        $this->write($node->getName());
    ...
    private function writeReindentedValue(CssDeclaration $node): void
    {
        ...
        if ($minimumIndentation === -1) {
            $this->buffer->write(StringUtil::trimAsciiRight($value, \true));
            $this->buffer->writeChar(' ');
            return;
        }
    ...
    private function writeCalculationValue(object $value): void
    {
        ...
        if ($value instanceof SassNumber && !is_finite($value->getValue())) {
            ...
            $parenthesizeRight = $right instanceof CalculationOperation && $this->parenthesizeCalculationRhs($value->getOperator(), $right->getOperator()) || $value->getOperator() === CalculationOperator::DIVIDED_BY && $right instanceof SassNumber && (is_finite($right->getValue()) ? $right->hasComplexUnits() : $right->hasUnits());
    ...
    public function visitColor(SassColor $value): void
    {
        ...
        } elseif ($name !== null && !NumberUtil::fuzzyEquals($value->getAlpha(), 0)) {
            $this->buffer->write($name);
    ...
    {
        $needsParens = $value instanceof SassList && ListSeparator::COMMA === $value->getSeparator() && !$value->hasBrackets();
        if ($needsParens) {
            $this->buffer->writeChar('(');
        }
        $value->accept($this);
    ...
    private function tryPrivateUseCharacter(SourceMapBuffer $buffer, string $char, string $string, int $i): ?int
    {
        ...
        if ($charCode >= 0xe000 && $charCode <= 0xf8ff || $charCode >= 0xf0000 && $charCode <= 0x10ffff) {
