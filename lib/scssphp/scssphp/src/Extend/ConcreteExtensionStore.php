...
final class ConcreteExtensionStore implements ExtensionStore
{
        ...
        foreach ($targets->getComponents() as $complex) {
            ...
            foreach ($compound->getComponents() as $simple) {
                ...
                foreach ($source->getComponents() as $sourceComplex) {
                    $extensionMap[$sourceComplex] = new Extension($sourceComplex, $simple, $span, optional: \true);
                }
     ...
     */
    private function __construct(SimpleSelectorMap $selectors, SimpleSelectorMap $extensions, SimpleSelectorMap $extensionsByExtender, \SplObjectStorage $mediaContexts, \SplObjectStorage $sourceSpecificity, \SplObjectStorage $originals, ExtendMode $mode)
    {
    ...
    private static function createForMode(ExtendMode $mode): self
    {
        ...
        return new self(new SimpleSelectorMap(), new SimpleSelectorMap(), new SimpleSelectorMap(), $mediaContexts, $sourceSpecificity, $originals, $mode);
    }
        ...
        foreach ($complex->getComponents() as $i => $component) {
            ...
            if ($extended === null) {
                if ($extendedNotExpanded !== null) {
                    $extendedNotExpanded[] = [new ComplexSelector([], [$component], $complex->getSpan(), $complex->getLineBreak())];
                }
            } elseif ($extendedNotExpanded !== null) {
                $extendedNotExpanded[] = $extended;
            } elseif ($i !== 0) {
                $extendedNotExpanded = [[new ComplexSelector($complex->getLeadingCombinators(), array_slice($complex->getComponents(), 0, $i), $complex->getSpan(), $complex->getLineBreak())], $extended];
            } elseif (\count($complex->getLeadingCombinators()) === 0) {
                $extendedNotExpanded = [$extended];
            } else {
                $newExtended = [];
                foreach ($extended as $newComplex) {
                    if (\count($newComplex->getLeadingCombinators()) === 0 || EquatableUtil::listEquals($complex->getLeadingCombinators(), $newComplex->getLeadingCombinators())) {
                        $newExtended[] = new ComplexSelector($complex->getLeadingCombinators(), $newComplex->getComponents(), $complex->getSpan(), $complex->getLineBreak() || $newComplex->getLineBreak());
                    }
                }
    ...
    private function extendCompound(ComplexSelectorComponent $component, SimpleSelectorMap $extensions, ?array $mediaQueryContext, bool $inOriginal): ?array
    {
        ...
        if ($this->mode !== ExtendMode::replace) {
            ...
            $result[] = new ComplexSelector([], [new ComplexSelectorComponent(new CompoundSelector(iterator_to_array(self::expandIterable($extenderPaths[0], function (Extender $extender) {
            ...
            }), \false), $component->getSelector()->getSpan()), $component->getCombinators(), $component->getSpan())], $component->getSpan());
        }
    ...
    private function unifyExtenders(array $extenders, ?array $mediaQueryContext, FileSpan $span): ?array
    {
        ...
        if ($originals !== null) {
            array_unshift($toUnify, new ComplexSelector([], [new ComplexSelectorComponent(new CompoundSelector($originals, $span), [], $span)], $span, $originalsLineBreak));
        }
    ...
    private function extenderForCompound(array $simples, FileSpan $span): Extender
    {
        ...
        return Extender::create(new ComplexSelector([], [new ComplexSelectorComponent($compound, [], $span)], $span), $this->sourceSpecificityFor($compound), \true);
    }
    /**
    ...
    {
        return Extender::create(new ComplexSelector([], [new ComplexSelectorComponent(new CompoundSelector([$simple], $simple->getSpan()), [], $simple->getSpan())], $simple->getSpan()), $this->sourceSpecificity[$simple] ?? 0, \true);
    }
    /**
    ...
    private function extendPseudo(PseudoSelector $pseudo, SimpleSelectorMap $extensions, ?array $mediaQueryContext): ?array
    {
        ...
        if ($pseudo->getNormalizedName() === 'not' && !IterableUtil::any($selector->getComponents(), fn($complex) => \count($complex->getComponents()) > 1) && IterableUtil::any($extended->getComponents(), fn($complex) => \count($complex->getComponents()) === 1)) {
        ...
        $complexes = iterator_to_array(self::expandIterable($complexes, function (ComplexSelector $complex) use ($pseudo) {
            ...
            switch ($pseudo->getNormalizedName()) {
                case 'not':
                    ...
                    if (!\in_array($innerPseudo->getNormalizedName(), ['is', 'matches', 'where'], \true)) {
                        return [];
                    }
    ...
    public function clone(): array
    {
        ...
        return [new ConcreteExtensionStore($newSelectors, $newExtensions, clone $this->extensionsByExtender, $newMediaContexts, clone $this->sourceSpecificity, clone $this->originals, ExtendMode::normal), $oldToNewSelectors];
    }
}
