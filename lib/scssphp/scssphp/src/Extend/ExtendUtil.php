...
use BlockstudioVendor\ScssPhp\ScssPhp\Ast\Selector\QualifiedName;
...
final class ExtendUtil
{
    ...
    public static function unifyComplex(array $complexes, FileSpan $span): ?array
    {
        ...
        $base = new ComplexSelector($leadingCombinator === null ? [] : [$leadingCombinator], [new ComplexSelectorComponent(new CompoundSelector($unifiedBase, $span), $trailingCombinator === null ? [] : [$trailingCombinator], $span)], $span, $hasLineBreak);
    ...
    public static function weave(array $complexes, FileSpan $span, bool $forceLineBreak = \false): array
    {
        if (\count($complexes) === 1) {
            ...
            return [new ComplexSelector($complex->getLeadingCombinators(), $complex->getComponents(), $complex->getSpan(), \true)];
        }
    ...
    private static function firstIfRootish(array &$queue): ?ComplexSelectorComponent
    {
        ...
        foreach ($first->getSelector()->getComponents() as $simple) {
            if ($simple instanceof PseudoSelector && $simple->isClass() && \in_array($simple->getNormalizedName(), self::ROOTISH_PSEUDO_CLASSES, \true)) {
                array_shift($queue);
                return $first;
            }
        }
        ...
        if ($combinator1 !== null && $combinator2 !== null) {
            ...
            if ($combinator1->getValue() === Combinator::FOLLOWING_SIBLING && $combinator2->getValue() === Combinator::FOLLOWING_SIBLING) {
                ...
                if ($followingSiblingComponent->getSelector()->isSuperselector($nextSiblingComponent->getSelector())) {
                    ...
                    $choices = [[$followingSiblingComponent, $nextSiblingComponent]];
                    if ($unified !== null) {
    ...
    public static function complexIsSuperselector(array $complex1, array $complex2): bool
    {
        ...
        while (\true) {
            ...
            if ($remaining1 === 1) {
                ...
                return self::compoundIsSuperselector($component1->getSelector(), ListUtil::last($complex2)->getSelector(), $component1->getSelector()->hasComplicatedSuperselectorSemantics() ? array_slice($complex2, $i2, -1) : null);
            }
    ...
    public static function compoundIsSuperselector(CompoundSelector $compound1, CompoundSelector $compound2, ?array $parents = null): bool
    {
        if (!$compound1->hasComplicatedSuperselectorSemantics() && !$compound2->hasComplicatedSuperselectorSemantics()) {
            ...
            return IterableUtil::every($compound1->getComponents(), fn(SimpleSelector $simple1) => IterableUtil::any($compound2->getComponents(), $simple1->isSuperselector(...)));
        }
        ...
        if ($tuple1 !== null && $tuple2 !== null) {
            return $tuple1[0]->isSuperselector($tuple2[0]) && self::compoundComponentsIsSuperselector(array_slice($compound1->getComponents(), 0, $tuple1[1]), array_slice($compound2->getComponents(), 0, $tuple2[1]), $parents) && self::compoundComponentsIsSuperselector(array_slice($compound1->getComponents(), $tuple1[1] + 1), array_slice($compound2->getComponents(), $tuple2[1] + 1), $parents);
     ...
     * @return SelectorList[]
     */
    private static function selectorPseudoArgs(CompoundSelector $compound, string $name, bool $isClass = \true): array
    {
        $selectors = [];
