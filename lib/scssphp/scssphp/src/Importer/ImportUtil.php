...
final class ImportUtil
{
    ...
    public static function resolveImportPath(string $path): ?string
    {
        ...
        if ($extension === '.sass' || $extension === '.scss' || $extension === '.css') {
            return self::ifInImport(fn() => self::exactlyOne(self::tryPath(Path::withoutExtension($path) . '.import' . $extension))) ?? self::exactlyOne(self::tryPath($path));
        }
        return self::ifInImport(fn() => self::exactlyOne(self::tryPathWithExtensions($path . '.import'))) ?? self::exactlyOne(self::tryPathWithExtensions($path)) ?? self::tryPathAsDirectory($path);
    }
    /**
    ...
    {
        $result = array_merge(self::tryPath($path . '.sass'), self::tryPath($path . '.scss'));
        if ($result !== []) {
            return $result;
        }
    ...
    private static function tryPathAsDirectory(string $path): ?string
    {
        ...
        return self::ifInImport(fn() => self::exactlyOne(self::tryPathWithExtensions(Path::join($path, 'index.import')))) ?? self::exactlyOne(self::tryPathWithExtensions(Path::join($path, 'index')));
    }
    /**
