...
final class ConcreteFileSpan extends SourceSpanMixin implements FileSpan
{
     ...
     */
    public function __construct(private readonly SourceFile $file, private readonly int $start, private readonly int $end)
    {
    ...
    public function union(SourceSpan $other): SourceSpan
    {
        ...
        } else if ($this->start > $other->getEnd()->getOffset() || $other->getStart()->getOffset() > $this->end) {
            throw new \InvalidArgumentException("Spans are disjoint.");
        }
        return $span;
    }
