...
namespace BlockstudioVendor\SourceSpan\Highlighter;

use BlockstudioVendor\SourceSpan\SimpleSourceLocation;
use BlockstudioVendor\SourceSpan\SimpleSourceSpanWithContext;
use BlockstudioVendor\SourceSpan\SourceSpan;
use BlockstudioVendor\SourceSpan\SourceSpanWithContext;
use BlockstudioVendor\SourceSpan\Util;
/**
...
final class Highlight
{
    ...
    public function __construct(SourceSpan $span, private readonly bool $primary = \false, ?string $label = null)
    {
    ...
    private static function normalizeContext(SourceSpan $span): SourceSpanWithContext
    {
        ...
        return new SimpleSourceSpanWithContext(new SimpleSourceLocation($span->getStart()->getOffset(), $span->getSourceUrl(), 0, 0), new SimpleSourceLocation($span->getEnd()->getOffset(), $span->getSourceUrl(), substr_count($span->getText(), "\n"), self::lastLineLength($span->getText())), $span->getText(), $span->getText());
    }
    /**
    ...
    private static function normalizeTrailingNewline(SourceSpanWithContext $span): SourceSpanWithContext
    {
        ...
        if (str_ends_with($text, "\n") && self::isTextAtEndOfContext($span)) {
            ...
            if ($text === '') {
                ...
                $end = new SimpleSourceLocation($end->getOffset() - 1, $span->getSourceUrl(), $end->getLine() - 1, self::lastLineLength($context));
    ...
    private static function normalizeEndOfLine(SourceSpanWithContext $span): SourceSpanWithContext
    {
        ...
        return new SimpleSourceSpanWithContext(
            $span->getStart(),
            new SimpleSourceLocation($span->getEnd()->getOffset() - 1, $span->getSourceUrl(), $span->getEnd()->getLine() - 1, \strlen($text) - Util::lastIndexOf($text, "\n") - 1),
            $text,
