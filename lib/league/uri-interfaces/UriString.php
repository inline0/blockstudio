...
use BlockstudioVendor\League\Uri\Idna\Converter;
use Stringable;
...
final class UriString
{
     ...
     */
    private const URI_SHORTCUTS = ['' => [], '#' => ['fragment' => ''], '?' => ['query' => ''], '?#' => ['query' => '', 'fragment' => ''], '/' => ['path' => '/'], '//' => ['host' => '']];
    /**
    ...
    {
        return self::buildUri($components['scheme'] ?? null, self::buildAuthority($components), $components['path'] ?? '', $components['query'] ?? null, $components['fragment'] ?? null);
    }
    /**
     ...
     * @link https://tools.ietf.org/html/rfc3986#section-7.5
     */
    public static function buildUri(?string $scheme, ?string $authority, string $path, ?string $query, ?string $fragment): string
    {
        $uri = '';
        if (null !== $scheme) {
            $uri .= $scheme . ':';
