...
use BlockstudioVendor\League\Uri\KeyValuePair\Converter;
use ReflectionEnum;
...
final class QueryString
{
    ...
    private const RECURSION_MARKER = "\x00__RECURSION_INTERNAL_MARKER__\x00";
    /**
     * @codeCoverageIgnore
     */
     ...
     */
    public static function compose(array|object $data, string $separator = '&', int $encType = PHP_QUERY_RFC1738, QueryComposeMode $composeMode = QueryComposeMode::Native): ?string
    {
    ...
    public static function composeFromValue(array|object $data, ?Converter $converter = null, QueryComposeMode $composeMode = QueryComposeMode::Native): ?string
    {
        ...
        $pairs = QueryComposeMode::Native !== $composeMode ? self::composeRecursive($composeMode, $data) : self::parseFromValue(http_build_query(data: $data, arg_separator: '&'), Converter::fromRFC1738());
     ...
     */
    private static function composeRecursive(QueryComposeMode $composeMode, array|object $data, string|int $prefix = '', SplObjectStorage $seenObjects = new SplObjectStorage()): iterable
    {
        QueryComposeMode::Safe !== $composeMode || is_array($data) || throw new TypeError('In safe mode only arrays are supported.');
        in_array($composeMode, [QueryComposeMode::EnumCompatible, QueryComposeMode::EnumLenient], \true) || !$data instanceof UnitEnum || throw new TypeError('Argument #1 ($data) must not be an enum, ' . ((new ReflectionEnum($data::class))->isBacked() ? 'Backed' : 'Pure') . ' given');
        if (is_object($data)) {
     ...
     * @throws SyntaxError
     */
    public static function extract(BackedEnum|Stringable|string|bool|null $query, string $separator = '&', int $encType = PHP_QUERY_RFC3986, QueryExtractMode $extractMode = QueryExtractMode::Unmangled): array
    {
        return self::extractFromValue($query, Converter::fromEncodingType($encType)->withSeparator($separator), $extractMode);
    }
    /**
     * Parses the query string.
     *
     ...
     * @throws SyntaxError
     */
    public static function extractFromValue(BackedEnum|Stringable|string|bool|null $query, ?Converter $converter = null, QueryExtractMode $extractMode = QueryExtractMode::Unmangled): array
    {
        ...
        return self::convert(self::decodePairs($pairs, self::PAIR_VALUE_PRESERVED), $extractMode);
    }
    /**
     ...
     */
    private static function extractPhpVariable(array $data, array|string $name, ?string $value = '', QueryExtractMode $extractMode = QueryExtractMode::Unmangled): array
    {
        if (is_array($name)) {
            [$name, $value] = $name;
