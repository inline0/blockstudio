...
use BlockstudioVendor\League\Uri\Idna\Converter as IdnConverter;
use Stringable;
use Throwable;
...
final class HostRecord implements JsonSerializable
{
    ...
    private function __construct(public readonly ?string $value, public readonly HostType $type, public readonly HostFormat $format)
    {
    }
    ...
    {
        return $this->isDomainName ??= match (\true) {
            HostType::RegisteredName !== $this->type, '' === $this->value => \false,
            null === $this->value => \true,
            default => is_object($result = IdnConverter::toAscii($this->value)) && !$result->hasErrors() && self::isValidDomain($result->domain()),
        };
    }
    public function ipVersion(): ?string
    {
        if (!$this->isIpVersionLoaded) {
            $this->isIpVersionLoaded = \true;
            $this->ipVersion = match (\true) {
    ...
    public static function from(BackedEnum|Stringable|string|null $host): self
    {
        ...
        if (null === $host) {
            return new self(value: null, type: HostType::RegisteredName, format: HostFormat::Ascii);
        }
        $host = (string) $host;
        if ('' === $host) {
            return new self(value: '', type: HostType::RegisteredName, format: HostFormat::Ascii);
        }
        ...
        if ($host === filter_var($host, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            return $inMemoryCache[$host] = new self(value: $host, type: HostType::Ipv4, format: HostFormat::Ascii);
        }
        if (str_starts_with($host, '[')) {
            ...
            if (1 === preg_match(self::REGEXP_IP_FUTURE, $ipHost, $matches)) {
                return !in_array($matches['version'], ['4', '6'], \true) ? $inMemoryCache[$host] = new self(value: $host, type: HostType::IpvFuture, format: HostFormat::Ascii) : throw new SyntaxError('The host ' . $host . ' is not a valid IPvFuture host.');
            }
            if (self::isValidIpv6Hostname($ipHost)) {
                return $inMemoryCache[$host] = new self(value: $host, type: HostType::Ipv6, format: HostFormat::Ascii);
            }
        ...
        if (1 === preg_match(self::REGEXP_REGISTERED_NAME, $domainName)) {
            return $inMemoryCache[$host] = new self(value: $host, type: HostType::RegisteredName, format: $format);
        }
    ...
    private static function isValidIpv6Hostname(string $host): bool
    {
        ...
        return 1 !== preg_match(self::REGEXP_NON_ASCII_PATTERN, $scope) && 1 !== preg_match(self::REGEXP_GEN_DELIMS, $scope) && \false !== filter_var($ipv6, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) && str_starts_with((string) inet_pton((string) $ipv6), self::ADDRESS_BLOCK);
    }
