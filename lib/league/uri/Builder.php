...
final class Builder implements Conditionable, Transformable
{
    ...
    public function host(BackedEnum|Stringable|string|null $host): self
    {
        ...
        if ($host !== $this->host) {
            null === $host || HostRecord::isValid($host) || throw new SyntaxError('The host `' . $host . '` is invalid.');
            $this->host = $host;
        }
        return $this;
    }
    ...
    public function port(BackedEnum|int|null $port): self
    {
        ...
        if ($port !== $this->port) {
            null === $port || $port >= 0 && $port < 65535 || throw new SyntaxError('The port value must be null or an integer between 0 and 65535.');
            $this->port = $port;
        }
        return $this;
    }
    ...
    public function authority(BackedEnum|Stringable|string|null $authority): self
    {
        ...
        return $this->userInfo($user, $pass)->host($host)->port($port);
    }
    /**
     * @throws SyntaxError
     */
    ...
    public function build(Rfc3986Uri|WhatWgUrl|BackedEnum|Stringable|string|null $baseUri = null): Uri
    {
        ...
        $uriString = UriString::buildUri($this->scheme, $authority, $path, Encoder::encodeQueryOrFragment($this->query), Encoder::encodeQueryOrFragment($this->fragment));
        return Uri::new(null === $baseUri ? $uriString : UriString::resolve($uriString, match (\true) {
    ...
    {
        if (null === $this->host) {
            null === $this->username && null === $this->password && null === $this->port || throw new SyntaxError('The User Information and/or the Port component(s) are set without a Host component being present.');
            return null;
        }
        $authority = $this->host;
    ...
    private function filterString(BackedEnum|Stringable|string|null $str): ?string
    {
        ...
        return UriString::containsRfc3987Chars($str) ? $str : throw new SyntaxError('The component value `' . $str . '` contains invalid characters.');
    }
}
