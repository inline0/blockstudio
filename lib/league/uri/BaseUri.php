...
use BlockstudioVendor\League\Uri\Contracts\UriAccess;
...
class BaseUri implements Stringable, JsonSerializable, UriAccess
{
     ...
     */
    final protected function __construct(protected readonly Psr7UriInterface|UriInterface $uri, protected readonly ?UriFactoryInterface $uriFactory)
    {
    ...
    private static function normalizedUri(Stringable|string $uri): Uri
    {
        ...
        return $uri->when(condition: '' === $uri->getPath() && null !== $uri->getAuthority(), onSuccess: fn(Uri $uri) => $uri->withPath('/'))->when(condition: null !== ($query = $uri->getQuery()) && str_contains($query, '&'), onSuccess: function (Uri $uri) use ($query) {
