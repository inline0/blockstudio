...
use BlockstudioVendor\League\Uri\UriTemplate\Template;
use BlockstudioVendor\League\Uri\UriTemplate\TemplateCanNotBeExpanded;
use BlockstudioVendor\League\Uri\UriTemplate\VariableBag;
use BlockstudioVendor\Psr\Http\Message\UriFactoryInterface;
use BlockstudioVendor\Psr\Http\Message\UriInterface as Psr7UriInterface;
use Stringable;
use BlockstudioVendor\Uri\InvalidUriException;
use BlockstudioVendor\Uri\Rfc3986\Uri as Rfc3986Uri;
use BlockstudioVendor\Uri\WhatWg\InvalidUrlException;
...
final class UriTemplate implements Stringable
{
     ...
     * @throws UriException if the resulting expansion cannot be converted to a UriInterface instance
     */
    public function expandToPsr7Uri(iterable $variables = [], Rfc3986Uri|WhatWgUrl|BackedEnum|Stringable|string|null $baseUrl = null, UriFactoryInterface $uriFactory = new HttpFactory()): Psr7UriInterface
    {
     ...
     * @throws UriException if the resulting expansion cannot be converted to a UriInterface instance
     */
    public function expandToPsr7UriOrFail(iterable $variables = [], Rfc3986Uri|WhatWgUrl|BackedEnum|Stringable|string|null $baseUrl = null, UriFactoryInterface $uriFactory = new HttpFactory()): Psr7UriInterface
    {
        $uriString = $this->templateExpandedOrFail($variables);
        return $uriFactory->createUri(null === $baseUrl ? $uriString : UriString::resolve($uriString, match (\true) {
            $baseUrl instanceof Rfc3986Uri => $baseUrl->toRawString(),
