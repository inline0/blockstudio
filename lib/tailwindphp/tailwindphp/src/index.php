 ...
 *     root: array,
 *     features: int,
 *     designSystem: \BlockstudioVendor\TailwindPHP\DesignSystem\DesignSystem
 * } Compilation result:
 ...
 * @return array{build: callable, buildExact: callable, sources: array, root: mixed, features: int, designSystem: \BlockstudioVendor\TailwindPHP\DesignSystem\DesignSystem}
 */
    ...
    walk($ast, function (&$node, $ctx) use (&$features, &$theme, &$utilitiesNodePath, &$sources, &$inlineCandidates, &$ignoredCandidates, &$firstThemeRule, &$important, &$customVariants, &$plugins, $options, &$seenFiles, &$seenVirtualModules) {
        ...
        if ($node['name'] === '@source') {
            ...
            if ($inline) {
                ...
                $parts = \BlockstudioVendor\TailwindPHP\Utils\segment($source, ' ');
                foreach ($parts as $part) {
                    foreach (\BlockstudioVendor\TailwindPHP\Utils\expand($part) as $candidate) {
...
class TailwindCompiler
{
     ...
     *
     * @return array{build: callable, buildExact: callable, sources: array, root: array, features: int, designSystem: \BlockstudioVendor\TailwindPHP\DesignSystem\DesignSystem}
     */
...
{
    return \BlockstudioVendor\TailwindPHP\Lib\Cva\compose(...$components);
}
