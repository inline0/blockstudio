...
* Fix: a cached prerendered page whose content-hashed Tailwind stylesheet is missing is re-rendered instead of served unstyled, through both the PHP cache and the Early Serve drop-in
* Fix: block attributes written at the top level, as the REST API, WP-CLI and content migrations produce them after discovering a block from the registry, are read at render instead of silently dropped in favour of the block's defaults; a value nested under `blockstudio.attributes` still wins (props @elliottmangham)
...
= 7.6.4 =
* Fix: every cache-path stat across the tailwind, runtime, build, prerender, warm queue, canvas, and early-serve code treats files removed by concurrent cleanups as stale instead of printing warnings into responses, and the early-serve drop-in regenerates
...
= 7.0.8 =
* Fix: resolve fatal error when installed via Composer

= 7.0.7 =
* Fix: use proper plugin icon on update screen

= 7.0.6 =
* Fix: add plugin icon to update screen

= 7.0.5 =
* Fix: test release for updater verification
