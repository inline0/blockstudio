...
class StaticPrerenderRuntimeTest extends TestCase {
	...
	protected function setUp(): void {
		...
		delete_option( 'blockstudio_static_prerender_feature_state' );
	}
	...
	public function test_disabled_static_prerender_registers_no_hooks_and_creates_no_state(): void {
		...
		try {
			...
			$this->assertFalse(
				has_action( 'blockstudio_static_prerender_warm', array( Static_Prerender_Runtime::class, 'run_cron_warmer' ) )
			);
			$this->assertFalse( get_option( 'blockstudio_static_prerender_feature_state', false ) );
	...
	public function test_disabled_prerender_subfeatures_register_no_operational_hooks(): void {
		$enable_prerender = '__return_true';
		add_filter( 'blockstudio/performance/staticPrerender/enabled', $enable_prerender );
		Runtime_Settings::reset();
		Static_Prerender_Runtime::deactivate();
		...
		try {
			...
			Static_Prerender_Runtime::deactivate();
			remove_filter( 'blockstudio/performance/staticPrerender/enabled', $enable_prerender );
			Runtime_Settings::reset();
		}
	}

	public function test_enabled_prerender_subfeatures_register_hooks_and_transition_state(): void {
		$enable_prerender = '__return_true';
		$enable_early     = '__return_true';
		$enable_warm      = '__return_true';
		add_filter( 'blockstudio/performance/staticPrerender/enabled', $enable_prerender );
		add_filter( 'blockstudio/performance/staticPrerender/earlyServe', $enable_early );
		add_filter( 'blockstudio/performance/staticPrerender/warm/enabled', $enable_warm );
		Runtime_Settings::reset();
		Static_Prerender_Runtime::deactivate();
		...
		try {
			...
			$this->assertNotFalse( wp_next_scheduled( 'blockstudio_static_prerender_warm' ) );
			$this->assertSame(
				array(
					'earlyServe'   => true,
					'warm'         => true,
				...
				get_option( 'blockstudio_static_prerender_feature_state' )
			);
		} finally {
			Static_Prerender_Runtime::deactivate();
			remove_filter( 'blockstudio/performance/staticPrerender/enabled', $enable_prerender );
			remove_filter( 'blockstudio/performance/staticPrerender/earlyServe', $enable_early );
			remove_filter( 'blockstudio/performance/staticPrerender/warm/enabled', $enable_warm );
			Runtime_Settings::reset();
		}
	}

	public function test_disabling_prerender_cleans_prior_side_effects_once(): void {
		$content = $this->root . '/transition-content';
		wp_mkdir_p( $content );
		file_put_contents( $content . '/wp-config.php', "<?php\n" );
		file_put_contents(
			$content . '/blockstudio-static-prerender-map.php',
			"<?php\n// Generated by Blockstudio. Static prerender early-serve map; do not edit.\nreturn array( 'entries' => array() );\n"
		);
		file_put_contents(
		...
		wp_schedule_single_event( time() + 3600, 'blockstudio_static_prerender_warm' );
		update_option(
			'blockstudio_static_prerender_feature_state',
			array(
				'earlyServe'   => true,
				'warm'         => true,
				'warmInterval' => 3600,
		...
		try {
			...
			$this->assertFileDoesNotExist( $content . '/blockstudio-static-prerender-map.php' );
			$this->assertFileDoesNotExist( $content . '/advanced-cache.php' );
			$this->assertFalse( wp_next_scheduled( 'blockstudio_static_prerender_warm' ) );
			$this->assertFalse( get_option( 'blockstudio_static_prerender_feature_state', false ) );
			$this->assertFalse(
				has_action( 'admin_init', array( Static_Prerender_Early_Serve::class, 'maybe_sync' ) )
