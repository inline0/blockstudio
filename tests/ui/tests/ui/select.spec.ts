...
test.describe('bsui/select', () => {
  ...
  test('ArrowDown navigates options', async ({ page }) => {
    const { root } = await openSelect(page);
    const options = root.locator('[role="option"]:not([aria-disabled="true"])');
  ...
  test('hidden form input updates on selection', async ({ page }) => {
    const root = page.locator('[data-bsui-select-root]');
    const trigger = root.locator('[data-bsui-select-trigger]');
  ...
  test('selected option has aria-selected=true', async ({ page }) => {
    ...
    await expect(selectedApple).toHaveAttribute('aria-selected', 'true');
  });
  ...
  test('selected option updates trigger text', async ({ page }) => {
    const { root, trigger } = await openSelect(page);
