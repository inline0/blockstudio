...
test.describe( 'interaction/confirmation-delete', () => {
	...
	test( 'clicking delete opens dialog with correct name', async ( { page } ) => {
		...
		await expect( page.locator( '[data-testid="dialog-message"]' ) ).toContainText( 'Project B' );
	} );
	...
	test( 'delete dialog shows correct name for different items', async ( { page } ) => {
		await jsClick( page.locator( '[data-testid="delete-btn"]' ).nth( 0 ) );
		await page.waitForTimeout( 300 );
		await expect( page.locator( '[data-testid="dialog-message"]' ) ).toContainText( 'Project A' );
		await page.locator( '[data-testid="cancel-btn"]' ).click();
		await page.waitForTimeout( 300 );

		await jsClick( page.locator( '[data-testid="delete-btn"]' ).nth( 2 ) );
		await page.waitForTimeout( 300 );
		await expect( page.locator( '[data-testid="dialog-message"]' ) ).toContainText( 'Project C' );
	} );
