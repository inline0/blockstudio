...
test.describe('component-richtext-default-twig', () => {
	test('add block', async () => {
		await addBlock(page, 'component-richtext-default-twig');
	...
	test('check content in editor', async () => {
		...
		canvas = await getEditorCanvas(page);
		await canvas.click('[data-type="blockstudio/component-richtext-default-twig"]');
		await count(canvas, 'text=Test text', 2);
	});
});
