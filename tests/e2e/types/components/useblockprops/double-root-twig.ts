...
test.describe('component-useblockprops-double-root-twig', () => {
	test('add block', async () => {
		await addBlock(page, 'component-useblockprops-double-root-twig');
	...
	test('check wrapper', async () => {
		...
		await count(canvas, '.is-root-container > .wp-block *', 3);
