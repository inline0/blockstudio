...
test.describe('component-useblockprops-innerblocks-richtext-twig', () => {
	...
	test('check wrapper', async () => {
		...
		await count(canvas, '.is-root-container > .wp-block > [contenteditable]', 1);
		await canvas.click('.is-root-container > .wp-block > [contenteditable]');
		await page.keyboard.type('Test text');
		await canvas.locator('text=Type / to choose a block').click();
		await page.keyboard.type('Test text');
