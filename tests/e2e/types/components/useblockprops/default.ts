...
test.describe('component-useblockprops-default', () => {
	...
	test('check wrapper', async () => {
		...
		await count(canvas, '.is-root-container > .wp-block > *', 2);
		...
		await canvas.click('[data-type="blockstudio/component-useblockprops-default"]');
		await openSidebar(page);
		await page.click('.block-editor-block-inspector__advanced');
		await page.locator('.block-editor-block-inspector__advanced input').nth(0).type('test-id', { delay: 100 });
	});
	...
	test('check content', async () => {
		...
		await count(page, '.wp-block-blockstudio-component-useblockprops-default', 1);
