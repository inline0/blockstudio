...
test.describe('component-useblockprops-innerblocks-default-twig', () => {
	test('add block', async () => {
		await addBlock(page, 'component-useblockprops-innerblocks-default-twig');
	...
	test('check wrapper', async () => {
		...
		await count(canvas, '.is-root-container > [data-is-drop-zone]', 1);
	...
	test('check content', async () => {
		...
		await count(page, '.wp-block-blockstudio-component-useblockprops-innerblocks-default-twig', 1);
	});
});
