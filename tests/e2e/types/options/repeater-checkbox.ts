...
testType('repeater-checkbox', false, () => {
  return [
      ...
      testFunction: async (page: Page) => {
        ...
        for (const checkbox of item.index) {
          await page
            ...
            .nth(index)
            .locator(`.components-checkbox-control:nth-of-type(${checkbox}) .components-checkbox-control__input`)
            .click();
        }
      },
    })),
    ...
    // Check persisted checkbox counts - repeater uses field selector, not panel body
    ...valuesCheckbox.map((item, index) => ({
