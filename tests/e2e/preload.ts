  ...
  test(`preloaded data contains all ${TOTAL_BLOCKS} blocks`, async () => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          bs?.blockstudioBlocks &&
          Object.keys(bs.blockstudioBlocks).length > 0
        );
      },
      { timeout: 30000 },
    );
    ...
    const data = await page.evaluate(() => {
      const blocks = (window as any).blockstudio.blockstudioBlocks;
      const entries = Object.values(blocks) as any[];
      return {
        count: entries.length,
        blockNames: [
          ...new Set(entries.map((e: any) => e.block?.blockName)),
        ],
    ...
    // Toggle blocks share hashes (only true/false), so unique count < total
    expect(data.count).toBeGreaterThanOrEqual(TOTAL_BLOCKS - 20);
  });
