...
A standalone migration command inventories an existing project and rewrites
shorthands to the canonical spelling. It is report-first, blocks on unresolved
cases, and changes only tag names. See
[Block Tag Migration](/docs/dev/cli#block-tag-migration) for the command, its
inputs, and its safety behaviour.
