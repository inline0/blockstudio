...
WordPress also discovers `.html` files recursively and normally exposes a
nested `index.html` under its path-derived slug, such as `header/index`.
When that file belongs to a Blockstudio manifest, Blockstudio removes the
path-derived duplicate and exposes only the manifest's declared `slug`.
Nested native `.html` templates without a Blockstudio manifest are unchanged.
