...
Fields listed in the block's `disabled` array are excluded from SEO analysis, matching frontend rendering behavior. When a field is [disabled](/docs/blocks/attributes/disabling), it is neither rendered on the frontend nor included in the SEO content.
