...
The code editor was useful in earlier versions for previewing blocks, but it
never reached full parity with a real development environment. Several plugin
features were held back over the years because they couldn't be supported
inside the standalone editor. It was also never going to be a replacement for
a full IDE. Removing it allows the plugin to focus entirely on file-based
