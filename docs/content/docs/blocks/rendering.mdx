...
stripped. This lets you add classes, IDs, or data attributes without them being
treated as block attributes.
...
The rendered output's root element receives `class="featured-card"`,
`id="main-card"`, and `data-analytics="card-click"`. Block attributes
(`title`) are passed to the block template as usual.
