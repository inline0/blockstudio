...
Each component receives the field metadata (`_fieldName`, `_fieldType`,
`_required`, `_enum`, `_format`) as additional attributes, so components can
adapt their rendering based on the schema.
