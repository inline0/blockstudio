...
Best for: prototyping, portable data, version-controllable datasets, config-like
data that should travel with the code.
...
Each CRUD operation has its own permission level. Use `true` for public
(unauthenticated) access, a WordPress capability string, an array of
capabilities (user needs any one), or omit entirely to require just
authentication.
...
This maps directly to WordPress's capability system. When a request lacks the
required capability, the endpoint returns a `403 Forbidden` response.
