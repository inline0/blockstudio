...
An alternative to keyed merging is connecting blocks to post meta using WordPress's [Block Bindings API](https://make.wordpress.org/core/2024/03/06/new-feature-the-block-bindings-api/). Instead of preserving content in block markup, the content lives in post meta. Template syncs can freely overwrite the markup because the editor reads from meta, not from innerHTML.
