...
Blockstudio includes built-in Tailwind CSS v4 support. When enabled, every frontend page is compiled server-side via [TailwindPHP](https://github.com/niclasmattsson/tailwindphp) with automatic file-based caching. In the block editor, a bundled Tailwind CDN script provides live preview as you edit.
