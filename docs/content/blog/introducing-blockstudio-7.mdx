...
As an alternative to keys, you can connect blocks directly to post meta using
WordPress's Block Bindings API. Content lives in meta instead of block markup,
so template syncs never destroy user edits. The data is also queryable via
`WP_Query` and available through the REST API.
