...
Blockstudio ships a static `blockstudio-llm.txt` containing the full v7
documentation and all JSON schemas in a single file (~48k tokens). The file is
pre-built at release time, not generated dynamically. Enable the
`ai/enableContextGeneration` setting and point any LLM coding assistant to
`your-site.com/blockstudio-llm.txt`. Works with Cursor, Claude Code, GitHub
Copilot, or any tool that accepts a URL or text file as context. See the
[AI documentation](/docs/ai) for details.
