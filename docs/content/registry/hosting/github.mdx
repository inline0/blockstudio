...
For private registries, users need to configure access. The CLI uses plain
`fetch()` without authentication headers, so private GitHub repos won't work
out of the box.

Options for private registries:
- Use a GitHub token in a proxy server
- Self-host behind your own authentication layer
- Use GitHub Packages or a custom server (see [Custom Server](/registry/hosting/custom-server))
