...
In ACF, field values are stored in post meta by default. In Blockstudio, field values are stored as block attributes in the post content. If you need values in post meta (for `WP_Query`, REST API access, etc.), add the `storage` property:
